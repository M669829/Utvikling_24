...
async function hentSaldo() {
    const response = await fetch("/auth/hentsaldo");
    const data = await response.json();

    if (response.ok) {
        document.getElementById("saldo").innerText = `Saldo: ${data.saldo}`;
    } else {
        console.error("Feil ved henting av saldo:", data.message);
    }
}
...
document.getElementById("trekke").addEventListener("click", async () => {
    const response = await fetch("/game/trekke", { method: "POST" });
    const data = await response.json();
...
document.getElementById("staa").addEventListener("click", async () => {
    const response = await fetch("/game/staa", { method: "POST" });
    const data = await response.json();
...
async function settInnsats() {
    ...
    const response = await fetch("/game/settInnsats", {
        method: "POST",
...
document.addEventListener("DOMContentLoaded", async () => {
    const response = await fetch("/game/hentSpilltilstand");
    const data = await response.json();
    ...
    if (data.aktivtSpill) {
        ...
        dealerValueDiv.textContent = `Dealerens synlige verdi: ${data.Dealerverdi}`;
        document.getElementById("saldo").innerText = `Saldo: ${data.saldo}`;
        document.getElementById("innsats").innerText = `Innsats: ${data.innsats}`;

        document.getElementById("trekke").disabled = data.spillAvsluttet;
        document.getElementById("staa").disabled = data.spillAvsluttet;
    }
});
